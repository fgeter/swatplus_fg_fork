      subroutine gwflow_phreatophyte() !rtb gwflow
...
!!    (extraction volumes are used in gwflow_simulate, in groundwater balance equations)
      ...
      if(gw_phyt_flag == 1) then
        ...
        !loop through the cells with phreatophytes
          ...
          gw_hyd_ss_yr(cell_id)%phyt = gw_hyd_ss_yr(cell_id)%phyt + et_Q !store for annual water
          gw_hyd_ss_mo(cell_id)%phyt = gw_hyd_ss_mo(cell_id)%phyt + et_Q !store for monthly water
      ...
      return
      end subroutine gwflow_phreatophyte
