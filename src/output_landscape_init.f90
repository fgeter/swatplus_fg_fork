...
!!! NEW SOILC_STAT/RESC_STAT/PLC_STAT CARBON OUTPUT FILES
 ...
 !! NEW BASIN CARBON ALL OUTPUT FILE
        ...
        if (pco%nb_hru%a == "y") then
          ...
          if (pco%csvout == "y") then
            call open_output_file(4569, "basin_carbon_all.csv", 1500)
            write (4569,*)  bsn%name, prog
            write (4569,'(*(G0.6,:,","))') bsn_carb_hdr
            write (4569,'(*(G0.6,:,","))') bsn_carb_hdr_units
            write (9000,*) "BASIN                     basin_carbon_all.csv"
          end if
        end if
