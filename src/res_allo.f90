      ...
      if (cs_db%num_tot > 0) then
        do ires = 1, sp_ob%res
          ...
          allocate (res_water(ires)%hmet(cs_db%num_metals), source = 0.)
          allocate (res_benthic(ires)%hmet(cs_db%num_metals), source = 0.)
          !rtb salt
          ...
          allocate (res_water(ires)%saltc(cs_db%num_salts), source = 0.)
          ...
          allocate (res_water(ires)%csc(cs_db%num_cs), source = 0.)
