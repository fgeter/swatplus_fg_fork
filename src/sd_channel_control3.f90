      ...
      !! Part 11: do NOT write back to the SHARED wst(iwst)%weat -- two channels sharing a
      !! weather station race on its allocatable components (ts/ts_next) -> double free.
      !! cli_lapse adjusted our threadprivate w in place, so use w%tave directly (equivalent).
