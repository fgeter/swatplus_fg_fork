...
!*** tu Wunused-label:       306 format (I8,4xA16, 10xA16,*(F16.4))                        ! format of chan_dr.swf
      ...
      !! SWIFT/ lives under out_path so -o (and -i without -o) place it with
      !! the other outputs.  out_path is empty when neither flag nor file.cio
      !! set it, so swift_dir is then just "SWIFT" in the current directory.
