...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
    ...
    !! if time of residence in reach is less than or eq to timestep, tres is
    !! too small (or zero) for the analytic solution below -- 1./tres would
    !! divide by zero / blow up. MJW 2023
      if (tres <= tdel) then
        wq_semianalyt = cint
      else
        help1 = 1. / tres - prock
